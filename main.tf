...
module "controlplane" {
  source           = "git::https://github.com/remche/terraform-openstack-rke2.git"
...
module "worker-gpu" {
  source           = "git::https://github.com/remche/terraform-openstack-rke2.git//modules/agent"
  boot_from_volume = true
  boot_volume_size = var.worker_volume_size
  boot_volume_type = var.worker_volume_type
  image_name       = var.image_name
  nodes_count      = var.num_workers_gpu
  name_prefix      = "worker-gpu"
  containerd_config_file = local.containerd_config_b64
  flavor_name      = var.worker_gpu_flavor_name
  server_group_affinity = "affinity"
...
module "worker-cpu" {
  ...
  containerd_config_file = local.containerd_config_b64
  flavor_name      = var.worker_cpu_flavor_name
  node_config      = module.controlplane.node_config
}
